...
ceiling of $1.0$, so within our bounded sweep (a ${\sim}25\times$ parameter increase,
...
alternative). Two parameter-matched pairwise dissociations (a three-cell partial factorial) separate
the factors; the two core effects (sharing helps, unreadability hurts) replicate across two held-out
query types at 20 seeds and in a larger three-object world, while the weak-perceptual edge over the
oracle is testbed-specific. Notably, the clean per-factor oracle is not the most sample-efficient
...
defeats linear decoding), replicated across two held-out query types. What the controls isolate
...
zero-shot compositional binding or transition generalization; robust to capacity, weight decay, and
training length; all failures occur strictly below a ceiling of $1.0$, and on transitions all
informative routes fall below even the \emph{blind} majority ceiling. (We read this as an inductive-bias limit for
...
dissociations (a three-cell partial factorial) separate them; the clean per-factor oracle is not
the most sample-efficient readable route; replicated across two held-out query types at 20 seeds and a three-object
...
whose one-hot is a single active index): the distributed codes transiently support composition
while the index-like codes go directly to lookup. Notably this grouping is \emph{different}
...
the answer-representation intact. It identifies the functional readout the converged model uses: it
reuses the readout for the trained shape slot rather than the held-out one (holding out ``shape of
left'' instead flips which slot is read). The claim is the intact-representation-plus-mis-wired-readout
...
($0.56$--$0.58$). We therefore state a narrower conclusion: a code that no probe (linear or
