...
\title{Input Pathways Shape How, Not Whether, Tiny Transformers Learn to Bind:\\
...
so every claim is evaluated on the entire input space (zero sampling variance), every informative
condition is information-theoretically matched (each such route uniquely determines the answer, a
fact we verify with exact Bayes ceilings), and every headline comparison is run at 10--20 seeds
with paired nonparametric tests (robustness arms use 5--20).
We report four findings. (1)~\emph{Endpoint invariance:} on held-out binding queries no route
yields converged zero-shot composition---every route ends at or below chance despite each route's
exact Bayes ceiling being $1.0$, so within our bounded sweep (a ${\sim}25\times$ parameter increase,
a $10\times$ learning-rate range, weight decay, and longer training) the failure is one of
inductive bias, not information. (A parallel transition-holdout diagnostic shows the same
memorization pattern; we treat it as weaker evidence, as the tiny world may not reward the rule.)
(2)~\emph{A two-factor account of few-shot binding:} when a small fraction of the held-out query
type is leaked into training, sample efficiency (in this testbed) is governed by
(i)~\emph{input-pathway parameter sharing} (a shared projection transfers to unseen query types;
per-factor embedding tables do not) and (ii)~\emph{readability} of the code (an entangled code that
probes cannot decode, raw linear decodability $0.95 \to 0.58$, is worse than every readable
alternative). A
two parameter-matched pairwise dissociations (a three-cell partial factorial) separate the factors;
the two core effects (sharing helps,
unreadability hurts) replicate across two held-out query types at 20 seeds and in a larger
three-object world, while the weak-perceptual edge over the oracle is testbed-specific. Strikingly,
the cleanest ``oracle'' code is the \emph{least} efficient readable route.
...
the converged output causally tracks the \emph{wrong} object slot and ignores the correct one---and
the entangled route inherits, rather than improves on, the readability of its input.
We also report, and replicate, the falsification of our own initial hypothesis that a grounded
side-channel induces compositional structure. All code, manifests, and per-seed logs will be released
...
noise or information mismatch can explain the difference.
...
We began this project expecting---and initially believing we had found---evidence that grounded
routes induce compositional structure. Our own adversarial control (an entangled perceptual code
that defeats linear decoding) falsified that hypothesis, and the falsification replicated. What
survived 20-seed replication across two held-out query types is, we believe, more useful than the
hypothesis it replaced: a mechanistic account of \emph{which properties of an input pathway matter,
for what, and why}, together with an unusually well-controlled negative result about zero-shot
...
\item \textbf{Endpoint invariance} (\S\ref{sec:endpoint}): no route achieves converged zero-shot
compositional binding or transition generalization; robust to capacity, weight decay, and training
length; all failures occur strictly below a ceiling of $1.0$, and on transitions all routes fall
below even the \emph{blind} majority ceiling. (We read this as an inductive-bias limit for
\emph{binding}, where the held-out parts are all present; for the transition task the same wording
is weaker---the world may be too small to reward the rule---so we scope that claim in \S\ref{sec:endpoint}.)
\item \textbf{A two-factor account of few-shot binding} (\S\ref{sec:fewshot}): sample efficiency is
governed by input-pathway parameter sharing and readability; two parameter-matched pairwise
dissociations (a three-cell partial factorial) separate them; the clean per-factor oracle is the
worst readable route; replicated across two held-out query types at 20 seeds and a three-object
world, and robust to the description order and to the specific synthetic mix.
...
(symbolic), systematic mis-binding with the answer decodable but the output causally reading the
wrong slot (index routes), and no readability gained over the input (entangled route).
\end{itemize}
...
relevant query space, so reported accuracies carry zero sampling variance and the only variance
source is model initialization.
...
Two scope notes on the statistics. \emph{Invariance is failure-to-reject.} The endpoint-invariance
and no-transition-difference claims are acceptances of the null; we read them as bounded (``no knob
in this box''), and note that the same $n$ that detected the $+0.05$--$0.08$ few-shot route effects
(Table~\ref{tab:fewshot}) would have detected zero-shot route differences of that size, so any
undetected route effect on the endpoint is smaller than the few-shot effects we do report.
...
Honest accounting: two effects we initially believed in did not survive this protocol. A three-way
...
(ceiling $1.0$) on every split: distinct states yield distinct inputs, so no route below fails for
...
\section{Results I: Endpoint Invariance---No Route Buys Zero-Shot Compositionality}
\label{sec:endpoint}
...
the three trained query types; 2-seed unconverged random-split sanity runs reach
$0.73$--$1.00$), so the failure is specific to composition. \textbf{Scope.} Because training
...
training regimes where memorization suffices, no route within our bounded sweep (${\le}2$ layers,
...
collapses less (Holm-corrected paired Wilcoxon, $n{=}20$). The grouping is by \emph{code format}: distributed codes (tokens, weak
and strong perceptual mixes) transiently support composition; index-like codes (per-factor tables,
one-hot through a shared linear) go directly to lookup. Notably this grouping is \emph{different}
...
type; distributed codes (blue/green/red) pass through a transient above-chance phase and collapse;
...
checkpoint, yet the converged model reads the wrong slot on up to $87\%$ of discriminating states---an intact representation with a mis-wired readout, and the mechanistic ground of the
...
strongly (index routes $0.71$--$0.86$; symbolic $0.51$)---the converged output causally reads the
wrong object. As a positive control, on a \emph{trained} query type the same intervention shows
every route causally tracking the \emph{correct} slot ($0.67$--$1.00$) and ignoring the other
($\approx 0.25$). What the intervention adds over the probe is real but bounded: it rules out
``decodable-but-causally-inert'' and shows the readout is functionally wired to the wrong slot with
the answer-representation intact. It does \emph{not} discover a surprising pathology---reading the
trained (left) position for a shape query is the expected reuse of the single shape-readout the
model ever built, and holding out ``shape of left'' instead (the \texttt{bind:1} arm) would flip
which slot is read. The value is the intact-representation-plus-mis-wired-readout dissociation, not
...
varied slot. On the held-out query the output causally tracks the \emph{wrong} slot and is
insensitive to the correct one; on a trained query it correctly tracks the queried slot---a direct
manipulation of the input, with no probe assumptions.}
\label{tab:causal}
\end{table}
...
\paragraph{The hypothesis we falsified.} We initially hypothesized that \emph{entangled} routes
...
for the weak code), so the $d{=}8$ saturating mix is \emph{practically} unreadable, not just
linearly so, and the trained model's residual readability ($0.59$) sits near this decoder ceiling
($0.56$--$0.58$). We therefore state only the weaker, safer form: a code that no probe (linear or
MLP) can decode is the code the model binds through worst, and the model never reads a code better
...
MLP-recoverable cell is not one we could construct with a fixed tanh mix (compression that killed
linear readability also killed information), and building it is open work.
...
factorial supporting two pairwise dissociations, not a full interaction estimate. Pooled over doses, one-hot-shared beats the
...
mechanism is, in hindsight, simple: a shared projection is trained by every query type and
therefore already implements the held-out one, while the held-out factor's private table receives
no readout-relevant gradient---weight-tying transfers, untied parameters do not
\citep{csordas2021devil}. We do not claim this is deep; the contribution is that sharing and code
\emph{format} were a priori confounded (the oracle is both index-like and modular) and the
parameter-matched onehot cell shows \emph{sharing}, not format, carries the effect---and that the
maximally clean per-factor code is thereby the \emph{worst} readable route, an instance of
...
\bottomrule
\end{tabular}
\caption{Few-shot binding, pooled advantage over the factored oracle (mean over doses per seed;
Wilcoxon vs.\ 0; $n{=}20$). The two-factor account: shared readable pathways $>$ modular readable
oracle $\gg$ shared unreadable code, replicated on a fresh held-out query type (the symbolic route
directionally, $p = 0.053$) and in a three-object world (\S\ref{sec:fewshot}, ``larger world'').}
\label{tab:fewshot}
\end{table}
...
core effects replicate: the shared one-hot pathway again beats the oracle (pooled $+0.066$,
...
advantage only when the code is nearly linearly readable, and a penalty when it is genuinely
entangled. The operative variables were mundane---pathway sharing and readability of a fixed
...
operationalized at two strengths; the strong code is \emph{injective} (exact ceiling $1.0$, so no
...
readout-swap or activation-patching evidence would further localize the mis-wiring. The symbolic
...
two-factor law predicts adapter- and prompt-style transfer in large models, where ``shared versus
...
exact commands are included in the supplementary material and will be publicly released upon
acceptance. Table~\ref{tab:spec} is the one-page experimental specification (routes, dimensions,
...
\section{Larger-World Replication (Three Objects)}
\label{app:bind3}
...
\midrule
$0.05$ & 0.68 & 0.72 & \textbf{0.81} & 0.71 & 0.45\\
$0.10$ & 0.94 & 0.93 & \textbf{0.98} & 0.94 & 0.51\\
\bottomrule
\end{tabular}
\caption{Three-object world, converged held-out balanced accuracy by route and dose ($n{=}10$,
chance $0.333$). Pooled vs.\ oracle: one-hot $+0.066$ ($p{=}0.037$); strong-entangled $-0.348$
($p{=}0.002$); weak-perceptual $-0.001$ ($p{=}0.77$, non-replicating).}
\label{tab:bind3}
\end{table}
...
$\sim$epoch~40. Full per-seed values are in the released manifests.
