...
inductive bias, not information. (A secondary transition-holdout diagnostic shows the same
...
type is leaked into training, sample efficiency (in this testbed) is best explained by
...
performance and saturates far below the readable alternatives; a dimension-matched control isolates
readability from input dimension). Two parameter-controlled pairwise dissociations (a three-cell
partial factorial) separate the factors; the two core effects (sharing helps, low readability hurts)
replicate across two held-out shape query types at 20 seeds and in a larger three-object world,
while the weak-perceptual edge over the oracle is testbed-specific. Notably, the clean per-factor oracle is not the most sample-efficient
...
best explained by input-pathway parameter sharing and readability; two parameter-controlled pairwise
dissociations (one exactly, one nearly parameter-matched) separate them; the clean per-factor oracle
is not the most sample-efficient readable route; replicated across two held-out shape query types at 20 seeds and a three-object
...
(Table~\ref{tab:fewshot}) are larger than the paired zero-shot route-difference confidence
intervals; we therefore treat endpoint invariance as a bounded empirical observation, not a formal
equivalence claim.
...
\paragraph{Readability, not input dimension.} The strong route is lower-dimensional ($d{=}8$) than
the readable routes ($d{=}16$), raising the confound that its few-shot deficit reflects input
dimension rather than readability. A dimension-matched control rules this out: a $d{=}16$ entangled
code (higher gain and depth) that is injective (exact ceiling $1.0$) yet poorly readable (raw linear
$0.57$, matching the $d{=}8$ code's $0.58$). At matched dimension, removing readability collapses
few-shot binding---this $d{=}16$ poorly-readable code pools $-0.294$ ($p=0.002$, $n{=}10$) below the
$d{=}16$ readable code and, if anything, sits slightly below the $d{=}8$ strong code
($-0.105$)---so readability, not input dimension, drives the deficit.
